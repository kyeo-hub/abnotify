...
Abnotify 是一款极简、纯净、安全的 Android 即时通知推送工具。它兼容 Bark 协议，采用创新的保活机制确保在各�?Android 系统（如 ColorOS、HyperOS 等）下都能稳定运行�?

## 核心特�?

- **Bark 兼容**：支持标准的 Bark 推送接口，您可以直接使用现有的推送脚本�?
- **内容加密传输**：服务器到客户端采用 RSA+AES 加密传输，保护推送内容隐私�?
- **超强保活**：利�?Android 辅助功能（Accessibility Service）作为保活锚点，配合独立进程守护，实现后�?永生"，告别消息延迟�?
- **Webhook 支持**：支�?GitHub、GitLab、Docker Hub、Gitea �?Webhook 通知�?
...
- **SQLite 存储**：存储设备密钥与 RSA 公钥的对应关系，用于设备注册和验�?
- **消息处理**：消息不做持久化存储，仅通过 WebSocket 实时转发
- **加密传输**：服务器收到推送后，用设备 RSA 公钥加密内容，再通过 WebSocket 发送给客户�?

**安全说明**�?

- **服务器可见明�?*：推送者发送的内容在服务器端是明文，这个除非用客户端到客户端之间发送，不然没有办法，但那样不就变成微信了吗....
- **设备验证**：通过 device_key 确保消息只发给指定设备，防止误发
- **加密传输**：服务器到客户端采用 RSA 加密，即�?WebSocket 被截获也无法解密
- **私钥保护**：每个设备的 RSA 私钥只存储在本地，确保只有目标设备能解密内容
- **私有服务器（推荐�?*：完全可控，适合敏感信息推�?
- **公共服务�?*：适合一般通知推送，不建议推送敏感内�?

### 2. 安装客户�?

1. 下载并安�?Abnotify APK�?
2. 打开应用，输入您的服务器地址�?
3. **重要�?* 进入"保活设置"，开�?辅助功能（保活）"和自启动权限�?
...
curl -X POST "http://your-server:8080/push/your-device-key" \
     ...
     -d '{"title":"Hello","body":"这是一条来�?Abnotify 的消�?}'
```
...
- **危险区域**：重�?RSA 加密密钥
- **关于项目**：查看版本信息、检查更新、访�?GitHub

## 保活说明

为确保应用后台稳定运行，请完成以下设置：

1. **辅助功能**：开�?Abnotify 辅助功能服务
2. **电池优化**：将 Abnotify 加入电池优化白名�?
3. **自启�?*：允�?Abnotify 开机自启动（部�?ROM 需要）
4. **后台运行**：允�?Abnotify 后台活动（MIUI/HyperOS 需要）
5. **通知权限**：允�?Abnotify 发送通知
6. **精确闹钟**：允许精确闹钟权限（Android 12+�?

## 注意事项

- 关闭前台通知后，保活能力会下降，建议仅在特殊情况下关�?
- 重置加密密钥后，旧消息将无法解密，请谨慎操作
- 服务器能见推送明文，敏感内容请使用私有服务器
- device_key 是唯一凭证，请妥善保管避免泄露
- Webhook 推送的消息会自动分组到 "webhook" �?
...
Abnotify 的灵感来自于 Bark 项目，并结合�?Android 无障碍服务保活的最佳实践�?
